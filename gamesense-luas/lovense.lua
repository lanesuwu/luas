--[[
    Lovenese Lua for gamesense.pub / skeet.cc
    
    Features:
    - Adjustable vibration intensity and duration
    - Support for multiple Lovense devices (because why not?)
    - Clan tag (flex your lovense)
    - Event triggers (kills, deaths, etc.)
    - Partner functionality for shared experiences
...
-- Duration Adjustment
local duration_toggle = ui_new_checkbox("Config", "Lua", "Adjust Duration")
local duration_slider = ui_new_slider("Config", "Lua", "Duration", 1, 10, 5)
...
local function log_vibration_message(duration, strength)
    local message = string_format("[Lovense] Vibration sent for %d seconds at %d strength.", duration, strength)
    client_exec("say " .. message) -- Send message to in-game chat cause funny
end

local function log_debug_message(...)
    local args = {...}
...
local function sex_toy(speed, time, special)
    ...
    http.get(url, function(success, response)
        ...
        --log_debug_message({247, 44, 133}, "[Lovense] ", {255, 255, 255}, status)
        return
    end)
end
...
local lovense = ui_new_button("Config", "Lua", "Connect to Lovense", function ()
    http.get("https://api.lovense.com/api/lan/getToys", function(success, response)
        ...
        if (response == nil or response[1] == nil) then log_debug_message({247, 44, 133}, "[Lovense] ", {255, 255, 255}, "No Devices found!") return end
        website = response[1][1]
        port = response[1][3]
        if website ~= nil and port ~= nil then
            log_debug_message({247, 44, 133}, "[Lovense] ", {255, 255, 255}, "Device has been paired!")
...
-- Menu items
local menu_items = { title, sep, data, space, options, toys, max, lovense_ind, lovense, clantag, partner, duration_toggle, duration_slider }
    ...
    if entity_get_local_player() ~= nil then
        ...
        if #vibration_queue > 0 then
            ...
            if not request[5] then
                log_vibration_message(request[3], request[2])
...
local function register_kill(event)
    ...
    local local_player = entity_get_local_player()
    local duration = ui_get(duration_toggle) and ui_get(duration_slider) or 1

    if contains(ui_get(options), 'On Death') and victim_index == local_player then
        table_insert(vibration_queue, {toy, ui_get(max), duration, globals_curtime(), false})
    ...
    elseif contains(ui_get(options), 'Kill') and killer_index == local_player then
        ...
        if time > 0.50 then
            ...
            table_insert(vibration_queue, {toy, strength_method, duration, globals_curtime(), false})
        end
...
local function handle_callbacks(state)
    ...
    -- Clantag Spammer (flex your lovense)
